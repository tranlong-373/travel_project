...
from .models import Accommodation, AccommodationReview, Room, PartnerAccommodationRequest


# ─── Form tùy chỉnh: nhập Khuyến mãi trực tiếp ───────────────────────────────
class AccommodationForm(forms.ModelForm):
    discount_percent = forms.IntegerField(
        ...
        min_value=0,
        max_value=100,
        help_text="Nhập % giảm giá, ví dụ: 8"
    )
    ...
    discount_amount = forms.IntegerField(
        ...
        required=False,
        min_value=0,
        help_text="Nhập số tiền giảm cố định, ví dụ: 300000. Ưu tiên hơn % nếu cả hai được nhập."
    )
...
class AccommodationAdmin(admin.ModelAdmin):
    ...
    def get_amenities_tags(self, obj):
        ...
        if isinstance(obj.amenities, list):
            tags_str = ", ".join([
                str(item) for item in obj.amenities
        ...
        sale_str = format_html(
            '<br><span style="color:red;font-weight:bold;">Sale: {}</span>',
            " | ".join(sales)
        ) if sales else ""
...
class RoomForm(forms.ModelForm):
    discount_percent = forms.IntegerField(
        label="Giảm giá phòng này (%)",
        required=False,
        min_value=0,
        max_value=100,
        help_text="Chỉ áp dụng riêng cho loại phòng này. Bỏ trống = dùng giảm giá chung của nơi lưu trú."
    )

    discount_amount = forms.IntegerField(
        label="Giảm giá phòng này (VND)",
        required=False,
        min_value=0,
        help_text="Giảm số tiền cố định riêng cho phòng này. Ưu tiên hơn % nếu cả hai được nhập."
    )
...
class RoomAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Giá & Khuyến mãi riêng phòng này', {
            ...
            'description': 'Để trống nếu muốn dùng khuyến mãi chung của nơi lưu trú.',
        }),
        ('Số lượng & Trạng thái', {
            'fields': (
                'total_rooms',
    ...
    def get_discount_tag(self, obj):
        ...
        source = "phòng" if (
            isinstance(obj.amenities, dict)
            and (
                obj.amenities.get('discount_percent')
                or obj.amenities.get('discount_amount')
            )
        ) else "chung"
...
# ─── Partner Accommodation Request Admin ─────────────────────────────────────
@admin.register(PartnerAccommodationRequest)
class PartnerAccommodationRequestAdmin(admin.ModelAdmin):
    list_display = (
        'hotel_name',
        'representative_name',
        'phone',
    ...
    @admin.action(description='Duyệt và thêm vào Accommodation List')
    def approve_requests(self, request, queryset):
        approved_count = 0
        skipped_count = 0

        for partner_request in queryset:
            if (
                partner_request.status != PartnerAccommodationRequest.STATUS_PENDING
                or partner_request.created_accommodation
            ):
                skipped_count += 1
                continue

            partner_request.create_accommodation()
            approved_count += 1

        self.message_user(
            request,
            f'Đã duyệt {approved_count} yêu cầu, tạo Accommodation và Room. '
            f'Bỏ qua {skipped_count} yêu cầu đã duyệt hoặc không hợp lệ.'
        )

    @admin.action(description='Từ chối yêu cầu')
    def reject_requests(self, request, queryset):
        updated = queryset.filter(
            status=PartnerAccommodationRequest.STATUS_PENDING
        ).update(
            status=PartnerAccommodationRequest.STATUS_REJECTED
        )

        self.message_user(
            request,
            f'Đã từ chối {updated} yêu cầu.'
        )
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change and obj.pk:
            old_obj = PartnerAccommodationRequest.objects.filter(pk=obj.pk).first()
            if old_obj:
        ...
        if (
            obj.status == PartnerAccommodationRequest.STATUS_APPROVED
            and old_status != PartnerAccommodationRequest.STATUS_APPROVED
            and not obj.created_accommodation
        ):
            obj.create_accommodation()