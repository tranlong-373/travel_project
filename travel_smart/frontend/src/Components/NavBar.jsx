    ...
    return (
        <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8">
                {navLinks.map((link, i) => (
                    <Link to={`${link.path}`} key={i} className={`group flex flex-col gap-0.5 ${isScrolled ? "text-gray-700" : "text-white"}`}>
                        {link.name}
            ...
            <div className="flex items-center gap-3 md:hidden">
                <img onClick={() => setIsMenuOpen(true)} src={assets.menuIcon} alt="menu-icon" className={`${isScrolled && 'invert opacity-80'} h-4`} />
            </div>
